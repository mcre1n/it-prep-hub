...
import { getAllTests } from '../lib/firebase';
...
export default function TestsPage() {
  ...
  const filteredTests = tests.filter((test) => {
    ...
    if (
      ...
      return false;
    if (difficulty !== 'all' && test.level !== difficulty) return false;
    return true;
  });

  const getCategoryCount = (categoryId) => {
    if (categoryId === 'all') return tests.length;
    return tests.filter((t) => t.category === categoryId).length;
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Библиотека тестов</h1>
          <p className="text-gray-600">
            {filteredTests.length} тестов для подготовки к собеседованиям
          </p>
        </div>
        ...
        {filteredTests.length === 0 ? (
          ...
          <div
            className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-300 ${animateCards ? 'opacity-0' : 'opacity-100'}`}
          >
